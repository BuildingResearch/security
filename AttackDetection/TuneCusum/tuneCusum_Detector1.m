...
%% Multiple simulations to tune CUSUM
nx = length(A);
nu = size(B);
nu = nu(1);
attack = ones(nu,1);
...
EI = 0;
for j = 1:nsim
    wg1= 2*pi + 2*pi*(10-1)*rand(1,1);    % Changes the frequency 
                % \in [1*2pi,10*2pi]
    So2= 2;
    zg=0.3 + 0.3*rand(1,1);    % zheta = 0.3, 0.6 (Hoang et al. paper), 0.041 0.491 0.168 (Almazan et al. paper)
                % \in [0.3, 0.6]
    p1 = 1;    % number of earthquakes
    for nn = 1:length(So2)
        wg=wg1;
        So=So2(nn);
        zwg=2*zg*wg;
    ...
    out = sim('B20_ATMD_NoEarthquake.slx');
    r = residues.data';
    res(:, :, j) = r;
end
%% Select parameters
